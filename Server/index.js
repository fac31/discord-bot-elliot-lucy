...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "update_task") {
    ...
    const filter = (i) =>
      ["complete_task", "reassign_task", "post_help"].includes(i.customId) &&
    ...
    collector.on("collect", async (i) => {
      if (i.customId === "complete_task") {
        ...
        const assigneeOptions = interaction.guild.members.cache.map(
          (member) => ({
        ...
        const filter = (i) =>
          i.customId === "select_assignee" && i.user.id === interaction.user.id;
        const collector = interaction.channel.createMessageComponentCollector({
          filter,
          time: 60000,
        });
        ...
        collector.on("collect", async (interaction) => {
          ...
          let selectedAssigneeUsername = assigneeOptions.find(
            (option) => option.value === selectedAssigneeId
          )?.label;
          selectedAssigneeUsername = { assignee: selectedAssigneeUsername };
          try {
            ...
            await interaction.reply({
              content: `**Task:** ${taskId} **Reassigned to:** ${taskData.assignee}.`,
              ephemeral: true,
            });
          } catch (error) {
